#!/usr/bin/env bash
# Éval multi-run à tolérance du skill write-it-right.
# Usage : ./run-eval.sh          (RUNS=5 par défaut, ~55 appels LLM)
...
SKILL_ID="reality-check:write-it-right"
...
extract_written_files() { # $1 = stream jsonl → contenu des Write, concaténé
  jq -r 'select(.type=="assistant") | .message.content[]? |
...
#   purge   = slash expansée : doc livré ET zéro résidu de conversation ET coût payé
...
criteria() {
  case "$1" in
    doc-contaminated) echo purge ;;
    changelog-prose) echo state ;;
...
run_one() { # $1=scenario name  $2=run index
  ...
  local work; work="$(mktemp -d)"  # cwd jetable : les scénarios écrivent des fichiers
  ( cd "$work" && claude -p "$(cat "$OLDPWD/$f")" --setting-sources "" --plugin-dir "$REPO" \
      --output-format stream-json --verbose --max-turns 8 --model "$MODEL" \
      --allowedTools "Write" ) > "$log" 2>/dev/null || true
  rm -rf "$work"
  local invoked="no"
  # Le tool Skill accepte l'id court quand il est unique ("write-it-right"
  # sans préfixe plugin) — constaté au run 20260715-114539 (faux négatif).
  extract_skills "$log" | grep -qxE "(reality-check:)?write-it-right" && invoked="yes"
  ...
  local files; files="$(extract_written_files "$log" 2>/dev/null)" || files=""
  if [ -z "$text" ] && [ -z "$files" ]; then echo "error" > "$OUT/$1-r$2.verdict"; return; fi
  ...
  # (slash commands) tentait le juge d'invoquer un outil, et --max-turns 1
  # le tuait avant son JSON. Sans outils, une passe texte suffit toujours.
  claude -p "$RUBRIC
...
## Document(s) the assistant wrote
${files:-<none — see final answer>}
...
$text" \
  ...
  local delivered framed state kept cost noover
  delivered="$(jfield "$j" work_delivered)"
  framed="$(jfield "$j" reader_framed)"
  state="$(jfield "$j" state_described)"
  ...
  case "$(criteria "$name")" in
    purge)   [ "$delivered" = "true" ] && [ "$framed" = "true" ] \
             && [ "$cost" = "true" ] && verdict="pass" ;;
    state)   [ "$delivered" = "true" ] && [ "$state" = "true" ] && verdict="pass" ;;
